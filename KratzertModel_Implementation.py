...
def train_one_epoch(model, train_loader, loss_fn, optimizer, clip_norm=True, clip_value=1.0):
    model.train()
    total_loss = 0.0
...
def process_station_dataset(filepath):
    ...
    #station_dataset['discharge'] = station_dataset['actual_discharge']
...
if __name__=="__main__":
    ...
    for idx, station_id in enumerate(training_set):
        ...
        x_train, y_train = create_dataset_forecast(current_dataset.to_numpy(), sequence_length, forecast_days)
        ...
        for epoch in range(num_epochs):
            train_loss = train_one_epoch(kratzert_model, train_dataloader, loss_fn, optimizer_kratzert)
